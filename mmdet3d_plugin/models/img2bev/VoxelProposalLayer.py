...
class VoxelProposalLayer(BaseModule):
    ...
    def create_grid(self):
        ...
        xs = (
            torch.linspace(0, ogfW - 1, ogfW, dtype=torch.float)
            .view(1, 1, ogfW)
            .expand(1, ogfH, ogfW)
        )
        ys = (
            torch.linspace(0, ogfH - 1, ogfH, dtype=torch.float)
            .view(1, ogfH, 1)
            .expand(1, ogfH, ogfW)
        )
    ...
    def depth2lidar(self, image_grid, depth, cam_params):
        ...
        points = torch.cat(
            [image_grid.repeat(b, 1, 1, 1), depth], dim=1
        )  # [b, 3, h, w]
        ...
        # cam to ego
        points = torch.cat(
            [points[:, :, 0:2, :] * points[:, :, 2:3, :], points[:, :, 2:3, :]], dim=2
        )
    ...
    def forward(self, cam_params, depth):
