...
class VoxFormerHead(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.volume_embed = nn.Embedding(
    ...
    def forward(
        ...
        **kwargs
        ...
        Args:
            ...
            proposal: Proposal volume from VoxelProposalLayer, which is generated by depth map
        ...
        bev_pos_cross_attn = self.positional_encoding(
            torch.zeros((bs, 512, 512), device=volume_queries.device).to(dtype)
        ).to(
            dtype
        )  # [1, dim, 128*4, 128*4]
        ...
        if self.mlp_prior is None:
            vox_feats_flatten[vox_coords[masked_idx, 3], :] = (
                self.mask_embed.weight.view(1, self.embed_dims)
                .expand(masked_idx.shape[0], self.embed_dims)
                .to(dtype)
            )
        else:
            vox_feats_flatten[vox_coords[masked_idx, 3], :] = self.mlp_prior(
                lss_volume_flatten[masked_idx, :]
            )
