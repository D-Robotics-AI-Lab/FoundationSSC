...
class VoxFormerEncoder(TransformerLayerSequence):
    ...
    def point_sampling(self, reference_points, pc_range, cam_params, img_metas=None):
        ...
        reference_points = reference_points.view(D, B, 1, num_query, 3).repeat(
            1, 1, num_cam, 1, 1
        )
        ...
        if bda.shape[-1] == 4:
            ...
            reference_points = torch.cat(
                (
                    reference_points,
                    torch.ones(*reference_points.shape[:-1], 1).type_as(
                        reference_points
                    ),
                ),
                dim=-1,
        ...
        if intrins.shape[3] == 4:
            ...
            reference_points_cam = (
                intrins.view(1, B, num_cam, 1, 4, 4) @ reference_points.unsqueeze(-1)
            ).squeeze(-1)
        else:
            reference_points_cam = (
                intrins.view(1, B, num_cam, 1, 3, 3) @ reference_points.unsqueeze(-1)
            ).squeeze(-1)
        ...
        reference_points_cam = post_rots[:, :, :2, :2].view(
            1, B, num_cam, 1, 2, 2
        ) @ reference_points_cam.unsqueeze(-1)
        reference_points_cam = reference_points_cam.squeeze(-1) + post_trans[
            :, :, :2
        ].view(1, B, num_cam, 1, 2)
        ...
        volume_mask = volume_mask.permute(2, 1, 3, 0, 4).squeeze(
            -1
        )  # [D, B, num_cam, num_query, 1] -> [num_cam, B, num_query, D]
...
class VoxFormerEncoder_DFA3D(VoxFormerEncoder):
    def __init__(
        ...
        **kwargs,
        ...
        super(VoxFormerEncoder_DFA3D, self).__init__(
            *args,
            pc_range=pc_range,
    ...
    def point_sampling(self, reference_points, pc_range, cam_params, img_metas=None):
        ...
        reference_points_cam[..., 0:2] = (
            post_rots[:, :, :2, :2].view(1, B, num_cam, 1, 2, 2)
            @ reference_points_cam[..., 0:2].unsqueeze(-1)
        ).squeeze(-1)
        reference_points_cam[..., 0:2] = reference_points_cam[..., 0:2] + post_trans[
            :, :, :2
        ].view(1, B, num_cam, 1, 2)
        ...
        reference_points_cam[..., 2] = (
            reference_points_cam[..., 2] - self.d_bound[0]
        ) / (self.d_bound[1] - self.d_bound[0])
