...
class PerceptionTransformer(BaseModule):
    ...
    def init_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, MSDeformableAttention3D) or isinstance(
                m, DeformSelfAttention
            ):
                try:
                    m.init_weight()
    ...
    def get_vox_features(
        ...
        **kwargs
        ...
        unmasked_ref_3d = (
            unmasked_ref_3d.unsqueeze(0).unsqueeze(0).to(unmasked_bev_queries.device)
        )
    ...
    def diffuse_vox_features(
        ...
        **kwargs
        ...
        unmasked_ref_3d = ref_3d[vox_coords[unmasked_idx, 3], :]
        unmasked_ref_3d = (
            unmasked_ref_3d.unsqueeze(0).unsqueeze(0).to(bev_queries.device)
        )
...
class PerceptionTransformer_DFA3D(PerceptionTransformer):
    ...
    def get_vox_features(
        ...
        **kwargs
        ...
        # To do, implement a function which supports bs > 1
        assert bs == 1
        bev_queries = bev_queries.unsqueeze(1).repeat(1, bs, 1)  #  # [N, 1, 128]
        bev_pos = bev_pos.flatten(2).permute(2, 0, 1)  # [N, 1, 128]
