...
from .multi_scale_3ddeformable_attn_function import (
    WeightedMultiScaleDeformableAttnFunction_fp32,
    WeightedMultiScaleDeformableAttnFunction_fp16,
    MultiScaleDepthScoreSampleFunction_fp32,
    MultiScaleDepthScoreSampleFunction_fp16,
    MultiScale3DDeformableAttnFunction_fp16,
    MultiScale3DDeformableAttnFunction_fp32,
)
...
class MSDeformableAttention3D_DFA3D(MSDeformableAttention3D):
    ...
    def init_smpl_off_weights(self) -> None:
        """Default initialization for Parameters of Module."""
        constant_init(self.sampling_offsets_depth, 0.0)
    ...
    def forward(
        ...
        **kwargs,
        ...
        value_dpt_dist = value_dpt_dist.view(bs, num_value, 1, dim_depth).repeat(
        ...
        sampling_offsets = torch.cat(
            [sampling_offsets_uv, sampling_offsets_depth], dim=-1
        )
        attention_weights = self.attention_weights(query).view(
            bs, num_query, self.num_heads, self.num_levels * self.num_points
        )
        ...
        if reference_points.shape[-1] == 3:
            ...
            sampling_locations_ref = reference_points.repeat(
                1, 1, num_heads, num_levels, num_points, 1, 1
            )
            assert num_all_points == num_points * num_Z_anchors
        ...
        weight_update = (
            (depth_score.mean(dim=-1) * attention_weights)
            .flatten(-2)
...
class DeformCrossAttention_DFA3D(DeformCrossAttention):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_empty:
            self.empty_query = nn.Embedding(
                self.bev_h * self.bev_w * num_head, embed_dims
            )
        ...
        **kwargs,
        ...
        for j in range(bs):
            for i, reference_points_per_img in enumerate(reference_points_cam):
                ...
                if self.use_empty:
                    empty_queries_rebatch[j, i, : len(index_query_per_img)] = (
                        self.empty_query.weight.view(
                            self.bev_h * self.bev_w, self.num_head, self.embed_dims
                        )[index_query_per_img]
                    )
        ...
        queries, update_weight = self.deformable_attention(
            query=queries_rebatch.view(bs * self.num_cams, max_len, self.embed_dims),
            key=key,
            value=value,
        ...
        if self.use_empty:
            queries = queries + ((1 - update_weight) * empty_queries_rebatch).mean(
                dim=-2
            )
        for j in range(bs):
            for i, index_query_per_img in enumerate(indexes):
